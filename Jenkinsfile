pipeline{
  ...
  stages{
    ...
    stage("Trigger CD pipeline"){
      steps{
        script{
          sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X -POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' -f 'IMAGE_TAG=${IMAGE_TAG}' 'http://172.28.73.189:8080/job/discord_update_image_tag/buildWithParameters?token=discord-token'"
        }
      }
    }
  }
